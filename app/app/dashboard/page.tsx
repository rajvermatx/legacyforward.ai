...
export default function DashboardPage() {
  ...
  return (
    <AppLayout>
          ...
          />
          {/* Career Snapshot */}
          <div className="bg-gradient-to-r from-navy-800 to-navy-900 rounded-2xl p-6 text-white mb-6">
            ...
            {hasOnboarded ? (
              ...
              <>
                ...
                <Link href="/app/onboarding" className="inline-block mt-4 px-5 py-2 bg-white text-teal-600 rounded-lg text-sm font-semibold hover:bg-indigo-50 transition">
                  Start Onboarding →
                </Link>
              </>
            )}
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <Link href="/app/wins" className="bg-white border border-slate-200 rounded-xl p-5 text-center hover:shadow-md transition">
              ...
              <div className={`text-3xl font-extrabold mt-1 ${wins.length > 0 ? "text-teal-600" : "text-slate-300"}`}>
                {wins.length || "0"}
              </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {hasOnboarded ? (
              ...
              <Link href="/app/onboarding" className="p-4 bg-indigo-50 rounded-xl text-center text-sm font-semibold text-teal-600 hover:bg-indigo-100 transition">
                💬 Start Onboarding
              </Link>
            )}
          ...
          {caii && (
            <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
              <div className="flex justify-between items-center mb-3">
                ...
                <Link href="/app/caii" className="text-xs text-teal-600 font-semibold hover:underline">View full report →</Link>
              </div>
          ...
          {wins.length > 0 && (
            <div className="bg-white border border-slate-200 rounded-xl p-5 mb-6">
              <div className="flex justify-between items-center mb-3">
                ...
                <Link href="/app/wins" className="text-xs text-teal-600 font-semibold hover:underline">View all {wins.length} →</Link>
              </div>
                ...
                {wins.slice(0, 3).map((win) => (
                  <div key={win.id} className="flex items-start gap-3 py-2 border-b border-slate-50 last:border-0">
                    <span className="text-xs text-slate-400 shrink-0 w-16">
                      <span suppressHydrationWarning>{new Date(win.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                    </span>
