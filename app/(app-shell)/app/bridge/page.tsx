            ...
            <div className="bg-white border border-slate-200 rounded-xl p-6">
              <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 items-end">
                <div>
                  ...
                  <input type="text" value={fromRole} onChange={(e) => setFromRole(e.target.value)} placeholder="e.g. Marketing Manager, Mechanical Engineer, Homemaker" className="w-full bg-slate-100 rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-teal-300 mt-1" />
                </div>
                <div className="text-2xl text-teal-600 font-bold text-center pb-2">→</div>
                <div>
                  <label className="text-xs text-slate-500 font-semibold">Where You Want to Go</label>
                  <input type="text" value={toRole} onChange={(e) => setToRole(e.target.value)} onKeyDown={(e) => e.key === "Enter" && analyze()} placeholder="e.g. Product Manager, DevOps Engineer" className="w-full bg-slate-100 rounded-lg px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-teal-300 mt-1" />
                </div>
              </div>
              <button onClick={analyze} disabled={loading || !fromRole.trim() || !toRole.trim()} className="w-full mt-4 py-3 bg-teal-600 text-white rounded-lg text-sm font-semibold hover:bg-teal-700 transition disabled:opacity-50">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {[
                  ...
                  <button key={f} onClick={() => { setFromRole(f); setToRole(t); }} className="text-xs px-2 py-1 border border-slate-200 rounded-full text-teal-600 hover:bg-indigo-50">
                    {f} → {t}
                  </button>
                ))}
              </div>
          ...
          {analysis && (
            <div className="space-y-4 mt-4">
              ...
              <div className="flex gap-4 items-center">
                ...
                <div className="text-3xl text-teal-600 font-bold">→</div>
                <div className="flex-1 p-4 bg-indigo-50 border-2 border-indigo-500 rounded-xl">
                  ...
                  <div className="text-lg font-bold text-teal-700 mt-1">{analysis.toRole}</div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div className="grid grid-cols-[1fr_30px_1fr] text-xs font-semibold">
                  ...
                  <div className="px-4 py-2 bg-indigo-50 text-teal-600 uppercase">How It Translates</div>
                </div>
                {analysis.transferableSkills.map((skill, i) => (
                  <div key={i} className="grid grid-cols-[1fr_30px_1fr] border-t border-slate-100">
                    ...
                    <div className="px-4 py-2.5 text-xs text-teal-700">
                      {skill.targetSkill}
                ...
                <div className="space-y-3">
                  {analysis.recommendedSteps.map((step) => (
                    <div key={step.step} className="flex gap-3">
                      <div className="w-7 h-7 rounded-full bg-teal-600 text-white flex items-center justify-center text-xs font-bold shrink-0">{step.step}</div>
                      <div>
                        <div className="text-sm font-semibold text-slate-700">{step.title}</div>
                        <div className="text-xs text-slate-500">{step.description}</div>
                        <div className="text-[10px] text-teal-600 mt-0.5">{step.timeframe}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex gap-3">
                <Link href="/app/roadmap" className="flex-1 py-3 bg-teal-600 text-white rounded-xl text-center text-sm font-semibold hover:bg-teal-700 transition">
                  📌 Generate Roadmap
                </Link>
