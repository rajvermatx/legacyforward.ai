...
Let me be upfront about something: this is not a machine learning textbook. You will not find yourself deriving the backpropagation algorithm on a whiteboard or spending chapters tuning hyperparameters to squeeze another half-percent of accuracy out of a model. There are excellent books that do exactly that, and they are written for data scientists, by data scientists, with the assumption that you want to spend your days inside a Jupyter notebook training models. Those books serve their audience well, and I have no interest in competing with them.
...
Every chapter in this book follows a consistent structure, and that is by design. When you are learning something new, the last thing you need is for every chapter to feel like a different book written by a different author. So here is what you can expect each time you turn to a new topic.

First, each chapter opens with the core concept explained in plain language, and — this is the part I am most proud of — mapped directly to Enterprise Architecture concepts you already understand. You will not find abstract descriptions floating in a vacuum. Instead, you will see how a new AI concept relates to patterns you have already worked with, whether that is a message bus, a caching layer, a data pipeline, or a governance framework. The goal is to give you anchors, so that every new idea connects to something solid in your existing mental model.
...
Every chapter in this book comes with a companion Jupyter notebook, and I want to explain why, because this was a deliberate choice that some readers might initially resist.
...
The goal here is emphatically not to turn you into a Python developer. If that happens along the way, wonderful, but it is a side effect, not the objective. The real goal is to give you enough hands-on experience with these technologies that your conversations with ML engineering teams become fundamentally different. Instead of asking “can we do this?” you will start asking “should we do this, and if so, what are the tradeoffs?” Instead of deferring to the data science team on every technical question, you will be able to engage as a genuine partner, bringing your architectural expertise to bear on problems that desperately need it. The notebooks are your lab, and every hour you spend in them pays dividends in the meeting rooms where the real decisions get made.
