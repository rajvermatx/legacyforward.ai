...
- The distinction between model metrics and business metrics and why it matters
- The metric hierarchy: from model performance to user behavior to business outcome
- How to set baselines before building so success can be measured
- The leading indicators that predict business impact before it arrives

## 19.1 The Metric Hierarchy

AI success metrics organize into three levels, each more meaningful than the one below:

**Level 1: Model metrics** — accuracy, precision, recall, latency, throughput. These measure whether the model is working correctly. They are necessary but not sufficient for measuring business success. A model metric that is degrading is a leading indicator of business impact, but a model metric that looks healthy does not guarantee business impact is being delivered.
