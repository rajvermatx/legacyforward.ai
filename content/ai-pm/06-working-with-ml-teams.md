...
The first step in closing the translation gap is having an accurate mental model of what the people you are working with actually spend their time on. This is not about learning ML. It is about understanding the workflow.
...
**Experimentation mode:** Testing different approaches, architectures, or configurations against the evaluation set to find what works. This is inherently iterative and often negative. Many experiments produce "we learned this does not work" rather than "we found the answer." Experimentation mode is the equivalent of the Explore phase, and its value is in narrowing the hypothesis space.

**Engineering mode:** Implementing the winning approach at production quality, including serving infrastructure, monitoring, API design, latency optimization, and integration. This is where ML work most closely resembles traditional software engineering, and it is where estimates become more reliable.

A common PM mistake is assuming the team is always in engineering mode, that they know what they are building and are executing against it. When a team is in data or experimentation mode, asking for commit dates creates pressure to prematurely exit exploration, which produces worse outcomes. The PM's job is to track which mode the team is in and apply the appropriate planning model.
...
**Do not ask:** "Can you add more training data?"
...
**Do not ask:** "How long will this take?"
**Do ask:** "What is the range of possible timelines for this work, and what are the main dependencies that would make it shorter or longer?"
...
**Do not ask:** "Will this reach 90% accuracy?"
...
The first framing requests a commitment the engineer cannot honestly give. The second surfaces the conditions under which the target is achievable, which is the information you actually need.

### Ask for trade-offs, not decisions

ML engineers often have strong intuitions about technical trade-offs that they do not surface unless asked, because they are uncertain whether it is their call to make. Ask explicitly for the trade-off landscape before making decisions.
...
**Ask:** "Where in this feature are you most uncertain? What is keeping you up at night technically?"

The last question is particularly valuable. ML engineers often carry technical concerns that they do not raise unless explicitly invited, because they are not sure the PM wants to hear them. Creating a norm of surfacing technical concerns early, rather than at the worst possible time mid-Harden, is one of the highest-value things a PM can do for an AI feature team.

## Why "Make It More Accurate" Is Not Actionable

"Make it more accurate" is the AI equivalent of "make it faster" or "make it better" in traditional software. It is directionally correct, but not actionable without significant translation work.

The reason it is not actionable is that "accuracy" for an AI feature is not a single number. It is a multidimensional performance profile with trade-offs between dimensions. Take a classification example.
...
Or: "The evaluation shows we are underperforming on tickets from enterprise customers compared to SMB customers. What would it take to close that performance gap?"
...
Three trade-offs come up in almost every AI feature delivery and require the PM's input to resolve. They are not technical decisions. They are product decisions that happen to have technical implications.
...
The product decision: what is the per-call cost at target volume, and does the unit economics of the feature support it?

A feature that costs $0.02 per user per day is sustainable. A feature that costs $0.50 per user per day might only work for enterprise pricing tiers. A feature that costs $0.001 per call but has latency so high that users lose confidence might not be worth the cost saving. The PM needs to understand the cost structure at target scale and evaluate whether it is compatible with the product's economics.
...
The product decision: given the cost of errors and the cost of human review, what is the right level of automation?
...
> **Think of it like this:** An AI copilot for an airplane assists with navigation and routine operations, but a human pilot is always in the loop for critical decisions. An AI that predicts whether a landing is safe does not make the landing. It informs the judgment of the person who does. How much of the "flying" you automate depends on the consequences of a wrong call, not on the raw capability of the AI.

## When the Data Scientist Says "It Depends"

"It depends" is the most frequent response to PM questions in AI development, and it is almost always honest rather than evasive. Here is a field guide to the most common "it depends" situations and how to move past them.

**"The accuracy depends on the data quality."**
What they mean: We have not fully characterized the data yet. If the data is as clean and representative as we hope, we can reach the target. If it has the quality issues we have seen in early exploration, we will fall short.
How to move forward: Prioritize a data quality assessment sprint before committing to accuracy targets. Ask: "What is the worst-case accuracy if the data quality issues we have identified do not improve? Is that above or below our launch threshold?"

**"The timeline depends on whether the first approach works."**
What they mean: We have a primary approach and an alternative. If the primary works, we will be done in X weeks. If it does not, we will need Y additional weeks to validate the alternative.
...
What they mean: We have evaluated on a test set, but we do not know if real user inputs will look like the test set. If they do, our quality metrics transfer. If they do not, we might see degradation.
How to move forward: Prioritize a production input sample analysis before full launch. Ask: "What is the minimum sample size we would need from a limited beta to characterize the production distribution?"
...
**Weekly sync with a standing agenda.** Not a status meeting. A working session. Agenda items: current phase status and gate timeline, emerging technical risks, trade-off decisions that need PM input, and blockers. Keep it to 30 minutes and make it recurring.

**Join evaluation reviews.** When the ML team reviews model outputs against the evaluation set, the PM should be in the room (or on the call). Seeing real output examples creates intuition that no summary document can provide. You will catch product issues, wrong tone, confusing explanations, edge cases the ML evaluation did not flag, that the ML team will not see because they are looking at different dimensions.

**Create a shared vocabulary.** The most productive PM-ML teams develop a shared language for the specific feature they are building. What does "good" look like for this specific output? What kinds of errors are "acceptable"? What does "distribution shift" mean in this specific context? Spending 30 minutes early in the Frame phase building a shared vocabulary on a whiteboard reduces ambiguity throughout the project.

**Treat ML engineers as the experts on feasibility, not on product decisions.** ML engineers can tell you what is achievable. They can model trade-offs. They can characterize risks. The product decisions, what accuracy level to require, what trade-off to accept, what edge cases to design around, when to launch, are yours to make, informed by their expertise. Be explicit about this division of responsibility. "I am asking you to tell me what is feasible and what the trade-offs are. The decision about which trade-off to accept is mine." This removes the ambiguity about whose call it is and allows ML engineers to surface information without feeling like they are making product commitments.
