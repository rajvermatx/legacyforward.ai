...
The most commercially significant form of AI right now is the **large language model** (LLM). Think of it as a system that has read an enormous fraction of human-written text — billions of documents, books, articles, websites, and records — and learned, with remarkable sophistication, how language patterns work. When you ask it a question, it generates a statistically likely continuation of that conversation based on everything it has read.

> **Think of it like this:** Imagine hiring a consultant who has read every business book, every industry report, every case study, and every analyst note ever published — but who has never actually run a company, signed a payroll check, or been responsible for a quarterly result. That consultant can synthesize, explain, draft, and summarize brilliantly. But they cannot tell you what your specific customers want. They cannot guarantee their cited data is accurate. And they cannot be held accountable in the way a named executive can. AI is that consultant.
...
The key insight is **scale**. A skilled human analyst can recognize patterns. AI can do it across a dataset 1,000 times larger, in minutes rather than months.
...
The value here is **throughput**. Tasks that required a junior analyst two days now take two minutes. That analyst can then focus on evaluation, judgment, and action rather than document processing.
...
Classification is one of AI's highest-confidence capabilities and often one of its best return-on-investment use cases. The cost per classification decision drops to near zero, and the system never has a bad Monday morning.

---

## The 5 Things AI Cannot Do

This section is the one your vendors are unlikely to walk you through. These are not temporary limitations that will be solved in the next release. They are structural properties of how current AI systems work.
...
The implication: every AI output that will be acted upon or shared externally requires a human review step. This is not optional, and any deployment plan that does not budget for it is not a realistic plan.
...
Consider a hiring decision. AI can screen resumes, flag relevant experience patterns, and even summarize interview transcripts. But the judgment call — whether this person will thrive in your culture, lead effectively under pressure, or represent the company well to clients — requires context, intuition, and accountability that AI cannot provide.
...
AI operates on text. It does not understand the unspoken context that shapes every real business situation: the political tension between two department heads, the fact that a key customer is on thin ice, the strategic reason your company made an apparently irrational decision three years ago.

This is why AI systems can produce responses that are technically accurate but profoundly wrong for the situation. They lack the institutional context that experienced humans carry. The more context-dependent a task, the more carefully AI outputs need to be filtered through human judgment.

### 4. Work Without Data

AI systems require data to function, and the quality of that data directly determines the quality of outcomes. This is such a consistent failure point that we devote a substantial portion of this book to it, but the headline version is:
...
AI models are trained on data up to a certain date. After that, they stop learning unless they are specifically updated. A model trained on data through mid-2024 does not know about regulatory changes from Q3 2025, your new product line launched last fall, or the market shift that happened in January.
...
**The demo environment has no legacy systems.** Your production environment connects to a CRM that was customized in 2017, an ERP on a version that your vendor hasn't supported in three years, and a data warehouse that was built by a team that is no longer with the company. The demo connected to clean, well-structured sample data.
...
When you ask a human analyst to run the same calculation twice, you get the same answer both times (assuming no errors). When you run the same SQL query twice, you get the same result. When you ask an AI the same question twice, you may get two different answers.

This is not a bug. It is a design characteristic. AI systems introduce a controlled degree of randomness in their outputs — called "temperature" in the technical literature — specifically because pure determinism would make their responses feel robotic and formulaic. A small amount of variability makes the outputs more natural.

The business problem is that you now have a system that does not reliably produce the same output from the same input. For audit purposes, for regulatory compliance, for quality control, and for customer trust, this matters enormously.

> **Think of it like this:** Imagine if your accounting software gave you a slightly different balance sheet each time you ran the month-end close, not because the data changed but because the system had a small random element built in. Your auditors would not accept it. Your board would not trust it. But this is exactly how raw AI systems behave, which is why production deployments require additional engineering to constrain and validate outputs before they are acted upon.

The implication for executives: any AI initiative that touches regulated outputs, financial reporting, legal documents, or externally visible content requires explicit controls to manage non-determinism. This is engineering work, and it has a cost.
...
**What specific capability is this using?** If the answer is vague ("it uses AI to improve operations"), push for specificity. Which of the four genuine capabilities is this leveraging? How does it do so?
...
**What data does it require, and do we have it?** If the answer assumes your data is cleaner or more connected than it actually is, the initiative will underperform.

**How does it behave when it is wrong?** All AI systems are wrong sometimes. The question is not whether the system will fail, but how it fails and what happens when it does. If the vendor has not answered this question clearly, you do not have enough information to commit.
...
These are not technical questions. They are the same due diligence questions you would apply to any business system. The fact that the system uses AI does not exempt it from standard evaluation rigor.

---

## The Bottom Line

AI is a genuinely powerful set of capabilities that is creating real competitive advantage in specific, well-understood applications. It is not magic, not omniscient, and not a replacement for human judgment. It is a tool — an exceptional one in the right context — that requires clear-eyed evaluation, proper deployment, and ongoing oversight.

The executives who will get the most value from AI in the next three years are not the ones who are most enthusiastic about the technology. They are the ones who understand it clearly enough to apply it where it genuinely excels, and disciplined enough to say no where it does not.
