...
The answer involves reliability, performance, cost of replacement, and risk.

**Reliability.** Modern cloud infrastructure is capable, but it is designed with the assumption that individual components will fail and the system will route around them. Mainframes are designed so that individual components almost never fail. For certain transaction types, including financial settlement, benefits processing, and airline reservations, the tolerance for failure is effectively zero. The mainframe was engineered for that tolerance. Replicating it with distributed cloud architecture is possible but requires significant engineering investment.
...
The practical implication for AI practitioners is this: if you are working at a large financial institution, a government agency, an insurance company, or any other organization that relies on mainframe transaction processing, the data that AI needs is probably in or flowing through that mainframe.

Getting that data into a modern AI system is not impossible, but it requires understanding how the mainframe exposes data, what format that data is in, and what the latency and volume constraints are. The mainframe does not have a REST API the way a modern cloud service does. It has its own interfaces, including batch file exports, VSAM data sets, and IBM MQ message queues, that require specific knowledge to work with.
