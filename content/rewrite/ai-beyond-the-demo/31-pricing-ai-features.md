...
**Generous free tiers are expensive.** Free AI feature tiers meant to drive adoption can become significant costs if the free usage volume is high. Budget caps must be built into free tiers from the start.

**Cost scales with capability.** More capable models are more expensive. A feature that uses a frontier model costs more than the same feature using a smaller model. Model selection is a cost decision as much as a capability decision.
