...
Data availability should be validated during discovery, not after. For each data source that the AI would require:
...
These artifacts are not the product specification. They are the evidence base for the decision about whether to proceed to specification. That decision — the "is this worth building?" question from Chapter 17 — should be made explicitly, with these artifacts as inputs, before scoping begins.
