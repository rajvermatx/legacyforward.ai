...
Begin with a disaster. Suppose you give a language model access to two tools — a web search function and a calculator — and ask it: *"What is the current population of Tokyo, and what percentage of Japan's total population does that represent?"*
...
Every ReAct agent follows the same three-phase cycle:
...
The decision point — "Is the task complete?" — is where most agent bugs live. Section 4.5 addresses stopping conditions in detail.

## 4.4 Building a ReAct Agent from Scratch

A complete ReAct agent in pure Python with no framework dependencies. The only external library is `openai`. This is deliberate: you need to understand every piece of the loop before you let a framework abstract it away.

### 4.4.1 The Tool Registry

First, a way to register tools and their schemas. A tool is any Python function that accepts string arguments and returns a string result. The schema tells the LLM what the tool does and what arguments it expects.
...
The `@tool` decorator does two things: it stores the function so we can call it later, and it stores the JSON schema so the LLM knows the tool exists. This separation — schema for the LLM, function for execution — is the foundation of every tool-use system, whether you build it yourself or use LangChain.
...
Walk through the critical design decisions.
...
With verbose mode on, you see the full reasoning trace: every thought, every tool call, every observation. An agent whose reasoning you cannot inspect is an agent you cannot trust.

### 4.4.4 Anatomy of a Trace

A successful trace for the population question looks something like this (abbreviated for clarity):
...
When the iteration limit is hit, `_force_final_answer()` asks the model to produce its best answer with whatever information it has gathered so far. A partial answer with a caveat is more useful than silence.
...
This works because the ReAct loop naturally handles new information — and an error is just new information. No special error-handling code is needed in the agent loop; the LLM handles it through reasoning.

### 4.6.2 Retry with Backoff

For transient errors (network timeouts, rate limits), wrapping tool execution in a retry is appropriate. Tool-level retries are invisible to the agent; agent-level retries consume iterations.
...
You might be wondering: why write all this from scratch when LangChain, LlamaIndex, CrewAI, and a dozen other frameworks offer ReAct agents out of the box?
...
Build from scratch until you hit a problem that a framework solves better than you can in an afternoon. Then adopt the framework for that specific capability, not as a wholesale replacement for understanding.
...
A useful agent is a configurable agent. Here is the version you will use for the chapter project — tools, the model, and the system prompt can all be swapped at initialization time:
...
Key improvements over the first version: tools are passed in as a dictionary rather than pulled from a global registry; `run()` returns both the answer and the full trace; stuck detection is built in. The trace is a list of dictionaries you can serialize to JSON, store in a database, or display in a UI.
