...
The demo looked flawless. A single agent that researched competitors, drafted a market analysis, pulled financial data, and produced a polished report — all in one long chain-of-thought. Then the team deployed it on real queries. The agent would start researching, get distracted by a tangent in the financial data, forget it was supposed to draft a report, and eventually time out after burning forty dollars in API calls. The problem was not intelligence. The problem was asking one agent to hold an entire workflow in its head at once.

The same principle that makes human organizations effective applies to agents: a coordinator who plans and delegates will outperform a single generalist who tries to do everything alone.
...
The supervisor-worker pattern addresses this by splitting the work into two distinct roles. The supervisor understands the overall goal, breaks it into subtasks, decides which worker handles each subtask, and assembles the final result. Workers are specialists: each has a narrow system prompt, a focused set of tools, and no knowledge of the broader workflow. A research worker does not know its output will be used in a report. A code-generation worker does not know its output will be reviewed by a quality worker. This isolation is a feature. It keeps each agent's context window focused on a single, well-defined task.

The analogy to human organizations is deliberate. A project manager does not write the code, design the UI, or draft the legal review. They decompose the project into work items, assign each to the right specialist, track progress, handle blockers, and synthesize deliverables. The supervisor agent does precisely the same thing.

> **Under the Hood: Supervisor vs. Orchestrator**
>
...
Each layer operates at a different level of abstraction. The supervisor thinks in terms of tasks: "research competitor pricing," "analyze trends in the data," "draft the executive summary." Workers think in terms of tool calls: "search the web for X," "query the database for Y," "format the output as a table." This separation keeps the supervisor's prompt strategic and each worker's prompt tactical.
...
The decomposition step is where the supervisor earns its keep. A good decomposition produces tasks that are independent where possible (enabling parallelism), correctly sequenced where necessary (using dependency declarations), and scoped to a single worker's capabilities.

> **Common Mistake: Decomposition Failures**
>
...
The worker design has deliberate constraints. The tool loop is capped at ten iterations to prevent runaway execution. Each tool call is wrapped in a try/except so that a single tool failure does not crash the worker — the error message is passed back to the LLM, which can decide to retry or work around the failure. The worker returns a structured dictionary, not free-form text, so the supervisor can programmatically check status and extract results.
...
> **Production Consideration: Tool Isolation Principle**
>
...
Task decomposition converts a vague user request into a directed acyclic graph of concrete, assignable tasks. The quality of decomposition determines the quality of the entire system's output.

Three common strategies:

**Sequential decomposition.** Tasks form a chain: each depends on the output of the previous one. Appropriate when later tasks genuinely need the results of earlier ones. Total latency equals the sum of all task durations.

**Parallel decomposition.** Independent tasks are identified and marked with no dependencies. They execute simultaneously, reducing total latency to the duration of the longest single task. The supervisor must identify which tasks are truly independent — falsely declaring a dependent task as independent leads to missing context, while falsely declaring an independent task as dependent wastes time.
...
The execution engine is a simple loop: find tasks whose dependencies are satisfied, execute them in parallel, collect results, repeat until all tasks are done or the plan is stuck. `ThreadPoolExecutor` is appropriate here because the bottleneck is I/O (API calls), not CPU computation.
...
Parallelism introduces three challenges that sequential execution does not have.
...
Once workers have completed their tasks, the supervisor combines their outputs into a coherent final response. This is often where the quality is won or lost.
...
The aggregation prompt must tell the supervisor to synthesize, not merely concatenate. Simply pasting worker outputs together produces a disjointed document. The supervisor should identify themes across worker outputs, resolve minor inconsistencies, apply the formatting the user requested, and flag any gaps caused by failed tasks.

> **Under the Hood: Aggregation Strategies**
>
...
> **Common Mistake: Circuit Breakers**
>
> Without a retry limit, a flawed task can loop indefinitely. Always cap retries and implement a circuit breaker. If a worker fails three times on the same task, mark it as permanently failed and let the supervisor aggregate without that result. A partial answer delivered in thirty seconds beats a complete answer that never arrives.
...
A complete, runnable example that creates a supervisor with three workers and executes a multi-step research and analysis workflow:
...
A production system would add logging at every step, persist task state to a database for recovery, implement streaming so the user sees progress, and include evaluation harnesses to measure the quality of decomposition, worker outputs, and final aggregation.
