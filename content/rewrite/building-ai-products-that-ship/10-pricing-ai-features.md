...
description: "Most software features cost almost nothing per additional user. AI features cost real money every time someone clicks. That changes everything about how you price them."
...
Traditional software features have a comforting economic property: once built, the marginal cost of an additional user using them is effectively zero. A user runs a search, applies a filter, exports a spreadsheet. The servers are running anyway. The engineering is amortized. The cost per interaction is fractions of a cent. This is why software businesses have such attractive unit economics — you build it once and the incremental usage is almost free.

![Diagram](/diagrams/ai-pm/ch10-1.svg)

AI features break this model entirely.

Every time a user invokes your AI feature, you pay. You pay for the tokens consumed by the model. You pay for the compute to run inference. You pay for the storage of context fed into the prompt. Depending on your architecture, you may also pay for vector database queries, embedding computations, and retrieval operations before the model starts generating. And you pay this cost every time, for every user, at every scale.

A feature that costs $0.50 per use sounds manageable when 100 users are using it. At 10,000 users, you're spending $5,000 a day in variable costs. At 100,000 users using it twice a day, you're spending $100,000 a day. The economics that worked as an experiment collapse at scale if you haven't priced for the cost structure.
...
Many AI features use Retrieval-Augmented Generation (RAG) or similar patterns that fetch relevant context before invoking the model. This retrieval has its own cost:
...
- **API gateway and monitoring**: Requests to model APIs go through infrastructure with its own cost.
- **Caching layer**: If you implement output caching, the cache itself has storage costs.
...
Run this accounting before you set a price. If you don't know your fully-loaded cost per call, you can't know whether you're pricing for margin or pricing for ruin.
...
- AI cost is a small fraction of overall COGS, typically under 5–10% of subscription revenue
...
Even "included" AI features should have soft usage limits that prevent extreme outliers from destroying your unit economics. A generous monthly limit that 95% of users never approach is better than no limit and a single customer who runs your AI feature as an automated pipeline and generates $50,000 in monthly cost against their $500/month subscription.
...
If your AI cost per monthly active premium user is $12, your premium tier needs to price for margin above that (e.g., $50/month premium vs. $25/month base) or cross-subsidize from account-level fees.
...
**When this breaks:**
- Usage-based pricing creates psychological friction that suppresses feature adoption — users hesitate because they're watching a meter
...
A hybrid approach works well in many cases: usage-based above a generous included baseline. Users get, say, 100 AI generations per month included, with additional usage at $0.10/generation. This preserves the simplicity and adoption benefits of the included model for most users while protecting economics at the tail.
...
This has happened to real products — ones that launched with optimistic cost assumptions and watched usage patterns diverge dramatically from projections.
...
- **AI cost as percentage of ARPU**: What fraction of what each user pays are you spending on their AI usage?
...
If your AI cost for any user segment exceeds 20–25% of the revenue they generate, you have a margin problem that will scale with adoption. Address it before the feature becomes a growth success and a financial problem simultaneously.
...
Never let a margin problem go unaddressed because the feature is popular. Popularity without economics is not success. It's a liability that grows with scale.
...
Because most software features have near-zero marginal cost at scale. AI features don't. Every free user who invokes your AI feature is a real cost that doesn't convert to revenue unless and until they pay.
...
- Free usage is unlimited, or the caps are generous enough to constitute the full product
- Conversion rates are low — free users are using the feature indefinitely without converting
- Your free tier is populated by users who have no intention of paying (students, hobbyists, competitors doing research)
- The free AI feature attracts high-volume automated usage that was never the target user

**The viability test for free AI tiers:** Estimate your cost per free user per month. Multiply by your free-to-paid conversion rate. Compare to the average revenue of a converted paid user over their first year. If the math doesn't work at current conversion rates, the free tier is not a growth strategy. It's a subsidy program.
...
You cannot price in a vacuum. Your pricing must be calibrated against what competitors charge and what the market has established as a reference point for AI value.
...
For each competitor, calculate what the user is getting per dollar: AI generations per dollar per month, documents processed per dollar per month, or whatever the relevant unit is for your category.
...
Competitive pricing analysis tells you what the market is charging. User research tells you what your specific users value and what they will pay. Run pricing sensitivity research — willingness-to-pay studies, conjoint analysis, or direct pricing conversations in user interviews — before setting final prices. Competitor data is an input, not a substitute for direct user signal.
...
Pricing AI features is not a one-time decision. Model costs will change. Competitor prices will change. Your usage patterns will evolve. Build a quarterly pricing review into your operating rhythm, and treat your AI feature economics as a living document rather than a launch deliverable.
