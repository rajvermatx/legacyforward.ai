...
slug: "glossary"
description: "Plain-language definitions for the most important terms in enterprise technology — the vocabulary you need to have informed conversations without needing to fake it."
...
**Change management** — The process of planning, testing, approving, and deploying changes to IT systems in a controlled way. In enterprise IT, change management exists to reduce the risk of changes causing problems in production.
...
**COBOL (Common Business-Oriented Language)** — A programming language created in 1959 that is still widely used in mainframe systems at banks, government agencies, and insurance companies. Old, but it is processing trillions of dollars in transactions every day.

**COTS (Commercial Off-The-Shelf)** — Software built by a vendor for general sale rather than custom-built for a specific organization. SAP, Salesforce, and Workday are COTS products.
...
**Data lake** — A storage system that holds large volumes of data in its raw, unprocessed form. Unlike a data warehouse, a data lake accepts any type of data — structured, semi-structured, or unstructured. Governance challenges frequently cause data lakes to become data swamps.
...
**Failover** — The automatic switching to a backup system or component when the primary fails. Enterprise systems with high availability requirements are designed with failover so that a single component failure does not take the whole system down.

**Firewall** — A security control that monitors and restricts network traffic. Firewalls are a fundamental component of enterprise security architecture, defining what traffic is allowed into and out of the network.

**General ledger** — The master financial record of an organization, capturing all financial transactions. The general ledger is maintained in the ERP or accounting system and is the authoritative source for financial reporting.
...
**IaaS (Infrastructure as a Service)** — The most basic cloud service model: the provider supplies virtual computing infrastructure (servers, storage, networking) that the customer configures and manages.
...
**Legacy system** — A system the organization cannot easily replace or significantly modify without unacceptable risk or cost. Legacy is defined by replaceability, not age.
...
**Point-to-point integration** — Direct connections between individual systems, built independently for each pair of systems that need to communicate. Works for small numbers of systems; creates spaghetti at scale.

**Procurement** — The business process of purchasing goods and services. Enterprise procurement is typically managed through the ERP and involves vendor selection, purchase orders, goods receipt, and invoice processing.
...
**Shadow IT** — Technology in use within an organization that is not managed or sanctioned by IT. Shadow IT emerges when official systems do not meet the needs of the people who need to work.

**Single source of truth** — The system or dataset designated as the authoritative record for a specific type of data. When only one source is authoritative, decisions can be made with confidence about which data to trust.
...
**SOA (Service-Oriented Architecture)** — An architectural pattern where software components provide services to other components through a network, using standard protocols. SOA was the precursor to modern microservices architecture.
...
**Technical debt** — Deferred investment in technology quality, architecture, or infrastructure that creates ongoing cost and risk. Like financial debt, it accrues interest over time.
...
**Vendor lock-in** — The condition where the cost of switching from a vendor's technology is high enough to constrain the organization's choices. Forms through data portability limitations, process dependencies, integration complexity, and skills concentration.
...
**VSAM (Virtual Storage Access Method)** — A file storage method used on IBM mainframes. VSAM datasets are a common way mainframe systems store and access data. Integrating with VSAM data requires mainframe-specific knowledge and tooling.
