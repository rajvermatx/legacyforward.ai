...
Cloud computing changed this by allowing organizations to rent computing infrastructure from large providers — primarily Amazon Web Services (AWS), Microsoft Azure, and Google Cloud Platform (GCP). Instead of buying a server, an organization can provision one in minutes, use it for as long as needed, and then shut it down and stop paying.
...
Most large enterprises do not run entirely in the cloud or entirely on-premises. They run in a hybrid: some systems in the cloud, some in their own data centers.
