...
- `WITH` is Cypher's way of piping results from one stage to the next (like a CTE in SQL)
...
`collect()` returns an actual list (array), not a concatenated string — useful when you need to process the list further.
...
The `WITH` clause computes an intermediate result that subsequent clauses can use. The difference is that `WITH` is inline — you do not need to declare it separately.
...
`MERGE` is Cypher's upsert: if the node exists, match it; if it does not, create it.
...
`DETACH DELETE` removes the node AND all its relationships. If you want to delete only the relationships and keep the node:
...
**Cypher:**
```cypher
MATCH (a:Account)-[t1:TRANSFERRED]->(b:Account)
...
Variable-length path traversal in one line.
...
In SQL, `FROM employees` reads the entire table (before WHERE filters it). In Cypher, `MATCH (e:Employee {name: 'Bob'})` uses an index on the `name` property (if one exists) to find Bob directly. Create indexes on properties you filter by:
...
Property access on non-existent properties returns NULL (it does not throw an error). This is like accessing a column that might not exist in a schema-less model.

```cypher
MATCH (e:Employee)
WHERE e.middle_name IS NOT NULL
RETURN e.name, e.middle_name
```

### Gotcha 4: No Implicit Cross Join

In SQL, `SELECT * FROM a, b` creates a cartesian product. Cypher does not have this by default. If you need a cartesian product:
