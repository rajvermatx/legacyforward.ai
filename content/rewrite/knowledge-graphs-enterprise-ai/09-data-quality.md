...
slug: "data-quality"
description: >-
  You have 50,000 nodes. Are they right? This chapter covers the six quality
  failure categories, automated checks for orphans and duplicates and type
  violations, semantic consistency rules, document coverage metrics, human
  review sampling strategies, drift monitoring, and a complete validation
  pipeline you can run after every ingestion batch.
...
In a relational database, the schema enforces correctness. NOT NULL constraints prevent empty required fields. Foreign keys prevent orphaned records. The database itself rejects a lot of bad data before it lands.

Graphs are different. There are no foreign keys: any node can connect to any other node. There is no NOT NULL constraint by default. A node can have any combination of properties. The most dangerous quality problems are not missing data but wrong relationships. A Person node connected by WORKS_AT to a Document node instead of an Organization node is syntactically valid — Neo4j accepts it without complaint. It is semantically wrong, and no automatic check will catch it unless you write one.

Testing a relational database is like proofreading a form: check that required fields are filled in and that dates look like dates. Testing a knowledge graph is like fact-checking a newspaper article: you need to verify not just that names and dates are present, but that the statements connecting them are true. "John Smith approved the vendor contract" is a claim. You need a way to verify it.
...
Nodes missing critical properties are less useful in queries and often indicate extraction problems.
...
Automated checks catch structural problems. Humans catch meaning problems. Both are necessary — automated checks are fast, human review is accurate, and you need both.
...
Quality is not a one-time check. As new documents are ingested and the graph grows, quality drifts. Monitor continuously to catch problems before they compound.
