...
Graph databases use different vocabulary for the same fundamental concepts. The fastest way to learn is to map one vocabulary to the other.
...
In a relational database, relationships are implied by matching values in columns. In a graph database, relationships are physical things — they exist as first-class objects, just like the data they connect. The difference between knowing that two people share a phone number (you have to look it up) and seeing them holding the same piece of string (the connection is visible and direct).
...
Here is an HR database expressed both ways.
...
// The junction table becomes a relationship WITH properties
...
1. **The `departments`, `employees`, and `projects` tables became nodes** with labels. Each row became a node. Each column became a property on that node.

2. **The foreign keys became relationships.** The `dept_id` column was replaced by a `BELONGS_TO` relationship. The `manager_id` self-referencing foreign key became a `REPORTS_TO` relationship. Foreign keys disappear entirely — they are not needed when the relationship is stored directly.

3. **The junction table disappeared.** The `employee_projects` table, which existed only to connect employees to projects, became a `WORKS_ON` relationship with `role` and `assigned_date` stored as properties on the relationship itself.

In the relational model, you describe connections with data ("Bob's manager_id is 101, and 101 maps to Alice"). In the graph model, you describe connections with structure ("Bob REPORTS_TO Alice"). The information is identical, but the storage is different — and that storage difference is what makes traversal fast.
...
The SQL requires four JOINs. The Cypher expresses the pattern directly: Bob works on a project, and someone else also works on that project.
...
When the relational database executes a JOIN, it performs a matching operation — takes a value from one table (`manager_id = 101`) and searches for that value in another table's index. This is a lookup: a search through a data structure.

When a graph database traverses a relationship, it follows a stored pointer. The relationship is not a value to be matched; it is an address to be followed. This is the fundamental mechanism behind the performance characteristics discussed in Chapter 1.

Step-by-step comparison for "find everyone in Bob's management chain up to the CEO":
...
Step 1 is the same — both databases need to find the starting node. But Step 2 is different. Instead of searching an index, the graph database reads a pointer from Bob's relationship list. That pointer is a direct memory address of Alice's node. The cost does not depend on how many employees exist in the database.

In the relational model, every hop in a traversal is like looking someone up in a phone book. In the graph model, every hop is like already having their phone number in your pocket. The phone book approach works, but it slows down as the phone book gets bigger.
...
For most teams starting with graph databases, **Neo4j** is the safest choice. Largest community, best learning resources, most mature tooling. Its query language (Cypher) is the de facto standard that other databases have adopted (openCypher). If you are already in the AWS ecosystem and want a managed service, **Amazon Neptune** is a reasonable choice, especially if you have RDF data or need Gremlin compatibility. **FalkorDB** is compelling if you need very low latency on smaller graphs and are already using Redis.

## 06. Property Graph vs. RDF: And Why It Matters for AI

You will encounter two fundamentally different graph models. Understanding the distinction matters as you move toward AI use cases.

### Property Graph

Nodes have labels and properties. Relationships have types and properties. Intuitive, flexible, and maps well to how most people think about data.
...
RDF represents everything as **triples**: subject-predicate-object. No properties on relationships. Everything is a node, and attributes are expressed as relationships to literal values.
...
The same data takes more triples to express, and the query language (SPARQL) has a steeper learning curve than Cypher. However, RDF was designed for **data interchange** and **ontology-based reasoning** — RDF graphs can link to external knowledge bases using standardized URIs, and OWL (Web Ontology Language) allows formal reasoning rules.
...
For the rest of this book, we focus on property graphs and Cypher.

## 07. The Data Model Is the Query

One of the most profound differences between relational and graph databases: in a relational database, the data model and the query language are separate concerns. You design your schema (DDL), then write queries against it (DML). Schema constrains what you can store; query language determines how you retrieve it.

In a graph database, the data model and the query language are nearly the same thing. When you write:
...
The practical implication: **if you can draw the answer on a whiteboard, you can write the query.** The Cypher query IS the whiteboard drawing, expressed in text. Relational queries describe operations (select these columns, join these tables, filter by these conditions) — you have to mentally translate between the ERD and the SQL. With Cypher, the translation step disappears.
...
The `REPORTS_TO*1..10` syntax tells Neo4j to follow the REPORTS_TO relationship between 1 and 10 hops. If Alice reported to a CTO, and the CTO reported to a CEO, all three would be returned with a single query and no recursive CTE.
