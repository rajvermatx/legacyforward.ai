...
slug: "graphrag"
description: >-
  Your RAG chatbot nails lookup questions. Ask it who approved the vendor
  whose part failed the safety test and you get silence. This chapter
  explains why vector-only retrieval breaks on relationship questions, and
  builds a complete GraphRAG pipeline with entity detection, graph traversal,
  hybrid context assembly, and question routing — including a 10-question
  comparison of vector-only vs GraphRAG accuracy.
...
Retrieval-Augmented Generation has become the default architecture for enterprise AI. Embed your documents, store them in a vector database, retrieve chunks by semantic similarity to the user's question, feed those chunks to an LLM. It works well for lookup questions — questions where the answer lives in a single chunk of text. "What is the vacation policy?" "How do I submit an expense report?" "What are the system requirements for Product X?"

It breaks for relationship questions.

When you embed the sentence "Acme Corp supplies the X-200 valve," you get a vector that captures something about a company, a product, and a supply relationship. When a user asks "Which vendors supply components used in Building 7?" the embedding of that question is semantically similar to chunks about vendors, components, and buildings. But the vector search has no way to connect the dots. Acme supplies the X-200, the X-200 is installed in the HVAC system, the HVAC system is in Building 7. Each of those facts might live in a different chunk, a different document, or a different system entirely. The vector search retrieves each in isolation. Nothing connects them.
...
GraphRAG adds graph traversal to the RAG pipeline. Instead of relying solely on vector similarity to find relevant context, it uses the structure of a knowledge graph to follow relationships between entities.
...
**1. Vector retrieval** — Same as standard RAG. Embed the question, retrieve similar chunks. This handles topic-level matching.

**2. Graph retrieval** — Identify entities in the question, then traverse the knowledge graph to find related entities, paths, and subgraphs. This handles structural matching.

**3. LLM synthesis** — Combine text chunks from vector retrieval with structured data from graph retrieval, and ask the LLM to synthesize an answer.

## 03. Microsoft's GraphRAG Approach

Microsoft Research published the most cited GraphRAG paper in 2024. Their approach introduced two important concepts worth understanding before you build your own implementation.

### Community Summaries

Microsoft's approach starts by building a knowledge graph from your documents (similar to Chapter 7), then runs a community detection algorithm (Leiden) on the graph to identify clusters of densely connected nodes. For each community, it generates a natural language summary using an LLM.
...
**Global search:** Given a broad question, retrieve community summaries across the entire graph. Good for thematic questions: "What are the main risks in our supply chain?"

## 04. Building a GraphRAG Pipeline Step by Step

A practical implementation using Neo4j for the graph, a vector store for embeddings, and Claude for synthesis.
...
Side-by-side across ten common question types. Quality is rated 1–5 (1 = fails, 5 = excellent).
...
Vector-only RAG is excellent for questions where the answer lives in a contiguous chunk of text. GraphRAG is essential for questions that involve connections between entities. In enterprise contexts, those are usually the most valuable questions.
...
The LLM detects "Building 7" in the question, but the graph stores it as "Facility B7" or "Building Seven." No graph results are found, and you silently fall back to vector-only results without knowing why.
