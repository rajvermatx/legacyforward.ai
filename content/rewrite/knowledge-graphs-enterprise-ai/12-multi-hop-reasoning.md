...
description: >-
  Who approved the vendor that supplies the component that failed the safety
  test? That is 4 hops. This chapter builds the full multi-hop reasoning
  pipeline: question decomposition, chain-of-traversal with intermediate
  results, single-query vs step-by-step tradeoffs, a complete reasoning
  agent with traceable output, and depth limit guidance by use case.
...
Multi-hop questions are the questions knowledge graphs were built to answer. They are also the questions that LLMs handle worst on their own. Each hop requires looking up specific data that no model has in its weights.
...
**Guessing the chain.** The LLM tries to answer the full chain from training data. "Who approved the vendor that supplies the failed component?" It generates a plausible-sounding answer: "Based on typical procurement processes, the VP of Supply Chain likely approved..." Fabrication, not reasoning. It has no access to your specific approval records.

**Partial retrieval.** With standard RAG, the LLM retrieves chunks related to the question. It might find a chunk mentioning the failed component and another mentioning vendor approvals. But the chunks do not connect — they come from different documents, different time periods, different contexts. The connection exists in the graph structure, not in any single text passage.
...
Each hop is verifiable. Each result is traceable. The final answer comes with a complete chain of evidence.

## 03. Decomposing Multi-Hop Questions

The first challenge is breaking a natural language question into individual hops. The LLM handles the language; the graph handles the data lookups.
...
Chain-of-traversal is the graph equivalent of chain-of-thought. Instead of reasoning through logic steps, the agent walks through graph steps — executing one traversal, observing the result, then executing the next traversal using that result.
...
Sometimes you do not need to decompose. When the path is predictable, you can traverse the full path in a single Cypher query and use the LLM to interpret the result.
...
A complete agent that handles multi-hop questions with a step-by-step trace:
...
Multi-hop queries can be expensive. The number of paths explored grows exponentially with depth:
...
Set hard depth limits based on your use case:
...
When traversing, restrict to relevant relationship types to narrow the search space:
