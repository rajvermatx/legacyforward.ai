...
description: >-
  Nobody is asking you to rip out Oracle. This chapter covers the sidecar
  pattern for adding graphs incrementally, a scoring framework for picking
  which data migrates first, dual-write vs CDC vs batch tradeoffs, a
  complete Debezium-to-Neo4j CDC pipeline, a four-phase migration timeline
  with exit criteria, team structure, and a feature-flag rollback strategy
...
Every year, someone proposes replacing the relational database with something new. Every year, that proposal dies. It should. Your relational database has 15 years of battle-tested stored procedures, 200 reports built on top of it, an operations team that knows how to restore it at 3am, and a vendor support contract that someone in procurement spent 6 months negotiating.
...
The rule of thumb: if your question starts with "find all things connected to X," that belongs in the graph. If it starts with "sum the revenue for Q3," that stays in the relational database.
...
Score each data domain on these 5 factors. A total score of 20 or above means migrate this first. Below 10 means leave it in the relational database for now.
...
Three ways to keep the graph in sync with the relational database. Each has trade-offs.
...
CDC reads the database transaction log (the write-ahead log, or WAL, in PostgreSQL) and streams every INSERT, UPDATE, and DELETE as an event. Debezium is the standard open-source CDC connector. It reads the WAL, publishes events to Kafka, and a consumer writes those events to Neo4j.
...
**Soft deletes in the graph.** When a row is deleted from PostgreSQL, the node in Neo4j is not deleted — it is marked as deleted. This preserves historical relationships and avoids orphaning connected nodes.

**MERGE, not CREATE.** Every write uses MERGE (upsert) so the pipeline is idempotent. If Kafka redelivers a message, the result is the same.

**One handler per table.** The decorator pattern keeps the mapping clean. Adding a new table means writing one function and registering it.

**Manual commit.** The Kafka offset is committed only after the Neo4j write succeeds. If the consumer crashes mid-write, it re-reads and re-processes the event on restart.
...
The most common mistake is assigning graph migration to one person. That person becomes a single point of failure and burns out. The minimum viable team is three people: one data engineer (pipeline), one developer (queries and application integration), one DevOps (infrastructure). Everyone else contributes part-time.
...
Because the graph is a sidecar and not the system of record, rollback is always the same: stop reading from the graph and go back to reading from the relational database. Every graph-powered query path needs a feature flag:
