...
A regional bank's fraud team manually investigates suspicious activity reports. An analyst gets an alert: Account A-7734 received an unusually large deposit and immediately sent smaller amounts to 8 different accounts. Today, that analyst spends 3–4 hours pulling transaction records, tracing where the money went, checking whether the receiving accounts are connected, and writing up findings.

This capstone automates the "follow the money" analysis.
...
The goal is a transaction graph and an agent that traces funds across multiple hops, detects common fraud patterns, and generates an investigation summary. What takes hours by hand takes seconds in the graph.
...
Variable-length paths let you follow the money to any depth:
...
The agent uses tool use to interact with the graph. The analyst asks a question; the agent decides which graph queries to run, executes them, and synthesizes the findings.
...
Add real-time alerting: run pattern detectors on new transactions as they arrive via CDC. Add a "similar case" tool that finds historical investigations with matching patterns. Build a visualization layer that renders the transaction graph for the analyst. Connect to the compliance knowledge graph from Capstone 1 to cross-reference regulatory requirements.

The manual investigation took 3–4 hours. The agent does the same traversal in seconds and produces a written report. That difference is the entire value proposition of putting transaction data in a graph.
