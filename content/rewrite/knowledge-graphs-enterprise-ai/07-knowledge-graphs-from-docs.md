...
description: >-
  Your documents have the answers. The problem is they are prose, not data.
  This chapter walks the full pipeline from PDF ingestion to a queryable
  graph: chunking, LLM extraction with Pydantic schemas, entity resolution
  across four strategies, and production scaling with checkpointing and
  parallel processing.
