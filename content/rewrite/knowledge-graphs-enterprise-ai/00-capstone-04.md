...
A support agent picks up a call. The customer is frustrated — bought a product 6 months ago, opened 3 support tickets, got a partial refund, and the same issue is back. The agent checks the CRM. Then the ticketing system. Then the order system. Three screens, three different search boxes, no connection between them.

This capstone builds the connection.
...
Three source systems, each with its own customer identifier:

1. **CRM (Salesforce export):** Accounts, contacts, opportunities, activities — keyed on `account_id`
2. **Support (Zendesk export):** Tickets, comments, satisfaction scores — keyed on `requester_email`
3. **Product/Orders (internal DB):** Orders, line items, products, returns — keyed on `customer_number`

The same customer appears as `ACC-4421` in Salesforce, `jane.doe@acme.com` in Zendesk, and `CUST-88912` in the order system. The goal is to unify these identities, build a customer graph, and provide natural language access to the full customer journey.
...
A warehouse can hold all this data. But answering "Show me the customer's full journey" in a star schema requires joining the customer dimension to the order fact table, then to the ticket fact table, then to the product dimension, then to the issue table. The graph makes it a single traversal from the customer node outward.

More importantly, the graph captures connections a warehouse misses:

- "This ticket is about the same product that failed for 15 other customers" (ticket → product → issue → other tickets)
- "This customer's support contact is also the decision-maker on a pending renewal" (ticket → contact → opportunity)
- "The product issue causing this ticket was fixed in version 3.2, but this customer is still on 3.1" (issue → product version → order → line item)
...
The hardest part of Customer 360 is stitching together the same customer across three systems that each use different identifiers.
...
These queries answer questions that previously required checking multiple systems.
...
Add real-time sync via CDC from Chapter 13 so the graph updates as CRM records change and tickets are opened. Build a proactive alerting system that flags at-risk customers daily. Add product version tracking to automatically identify customers running affected versions. Connect to the compliance knowledge graph from Capstone 1 for regulated customer communications.

Three screens became one call. That is what the graph gives a support agent on a live call — not more data, but the right data already connected.
