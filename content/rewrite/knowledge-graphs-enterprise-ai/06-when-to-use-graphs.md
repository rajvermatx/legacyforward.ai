...
Graph databases have been gaining momentum since 2020. The arrival of GraphRAG and knowledge-graph-powered AI in 2024–2025 pushed them further into the spotlight. Articles with titles like "Why Every Enterprise Needs a Knowledge Graph" are making the rounds.
...
**If no:** Endpoint queries are well-served by relational JOINs.

### Question 3: Is the relationship itself important (not just the entities)?

**What this means:** Do you care about attributes of the connections between entities? "Alice manages Bob" is a relationship. "Alice has managed Bob since 2022 with a performance rating of 4.5" is a relationship with properties.
...
**What this means:** Are you looking for structural patterns in your data? "Find all triangles of companies that trade with each other." "Find all employees who report to someone in Department A but work on a project led by someone in Department B."
...
**If yes:** The schema flexibility of graphs is a significant advantage when your data model is evolving rapidly.
...
Fraudsters create networks of fake accounts that transact with each other to move money, inflate reviews, or manipulate systems. These networks are invisible when you look at individual transactions but obvious when you look at the connections between accounts.
...
SQL equivalent: 20–40 lines with multiple self-joins. Often infeasible at scale because the query planner cannot optimize the circular join pattern.

### Use Case 2: Supply Chain Dependency Mapping

When a disruption occurs, you need to know which products are affected — including indirect dependencies through sub-suppliers. Supply chains are inherently graph-structured, requiring variable-depth traversal.
...
Recommendation engines rely on collaborative filtering — finding similar entities based on shared connections. This is a traversal problem: start at the user, find what they are connected to, find other users who share those connections, then find what those users are connected to that the original user is not.
...
Regulatory compliance requires tracing the full chain of actions, approvals, and entities involved in a decision. The graph naturally models the paper trail that compliance teams need to trace.
...
A knowledge graph structures your organization's information as a network of connected facts. When an AI agent needs to answer a complex question, it traverses this network to find and assemble the answer.
...
**Standard CRUD applications.** User registration, profile management, content management systems, inventory tracking — these are tabular workloads with well-defined schemas and simple queries.

**Financial reporting and analytics.** Revenue reports, budget tracking, quarterly summaries — aggregation-heavy workloads. SQL's GROUP BY, window functions, and ROLLUP are purpose-built for this. The tooling (BI platforms, reporting frameworks) is built around SQL.

**Time-series data.** IoT sensor readings, application logs, stock prices — neither relational nor graph databases are optimal. Use a purpose-built time-series database (InfluxDB, TimescaleDB, Prometheus).

**Simple transactional systems.** Point-of-sale systems, booking engines, order processing — workloads where the primary concern is ACID compliance, throughput, and consistency.

**Batch ETL and data warehousing.** SQL-based ETL tools (dbt, Informatica, SSIS) dominate this space. Graph databases can participate as a target, but they are not the right orchestration layer for batch data movement.
...
Each database handles what it does best. The application routes queries to the appropriate database based on query type. **Best for:** Greenfield architectures or microservices.
...
The knowledge graph sits between the AI layer and the transactional databases. **Best for:** Adding AI capabilities to an existing enterprise application.
...
Fully managed, no ops overhead, good free tier for prototyping. Can get expensive at scale.
...
Fully managed on AWS, integrates with IAM and CloudWatch. I/O charges can be unpredictable.
...
Full control, lowest per-unit cost, no vendor lock-in. You own the uptime, backups, upgrades, and security patches.
...
- [ ] Evaluate whether the graph should become a system of record for relationship data
