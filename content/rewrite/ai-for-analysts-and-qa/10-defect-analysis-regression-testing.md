...
description: "Defect backlogs grow faster than teams can triage them. Regression suites grow faster than teams can maintain them. LLMs can bring order to both — classifying and prioritizing defects, then generating, healing, and analyzing regression tests automatically."
...
| test\_search\_results\_count | RETIRE | 40% failure rate, never caught a real bug, 7 maintenance events. The test validates a dynamic count that changes with data, not actual search functionality. Replace with test\_search\_returns\_relevant\_results. |
