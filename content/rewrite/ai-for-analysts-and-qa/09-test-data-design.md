...
description: "Good tests are only as good as the data behind them. Here is how to use LLMs to generate realistic synthetic test data, discover the edge cases that manual data misses, build persona-driven scenarios, and stay compliant with GDPR, HIPAA, and CCPA — without ever touching production databases."
...
Good tests are only as good as the data behind them. In this chapter you will learn how to use LLMs to generate realistic synthetic test data, discover hidden edge cases, build persona-driven test scenarios, and handle data privacy — all without touching production databases.
...
Ask any QA team what slows them down, and test data ranks near the top. Production data cannot be used because of privacy regulations (GDPR, HIPAA, CCPA). Manually created test data is too clean — it lacks the messiness of real-world data. Generated data from simple random functions lacks semantic coherence: a randomly generated "customer" might have a phone number from New York and an address in Tokyo.
...
LLMs address these challenges because they understand that a valid US Social Security number has the format XXX-XX-XXXX, that "123 Main Street" is a plausible address, and that a person born in 1990 should not have a retirement date in 2015. This semantic understanding produces test data more realistic than random generation.

One caution: because LLMs are trained on internet text, they may occasionally generate real names, addresses, or phone numbers that belong to actual people. Always add a post-processing step that validates generated data is not accidentally real — check names against known public figures, verify phone numbers are from designated test ranges, and use clearly synthetic identifiers.

## 10.2 Synthetic Data Generation

The core technique is to describe your data schema and domain constraints in plain language, then ask the LLM to generate records that are realistic but entirely fictional. The prompt specifies rules for internal consistency (ages matching birth dates, addresses being plausible), realistic distribution (not all values identical), and natural imperfections (optional fields sometimes empty, varied formats). Use temperature 0.8 to encourage variety across records.

For an insurance application customer schema with 16 fields, the LLM generates data like:
...
Notice the semantic coherence: the phone area code (312) matches Illinois, the zip code (60540) is valid for Naperville, and the premium and risk score are reasonable for a home policy. A purely random generator would not produce this.

### Scaling Beyond LLM Token Limits

For large datasets, you cannot generate everything in a single LLM call. The strategy: use the LLM to generate a diverse seed set (10–20 records), then expand programmatically by cloning seed records with random variations in numeric fields, optional fields, and status values.

For even more realistic results, generate 5 batches of 20 seeds with different prompts ("young urban customers," "rural elderly customers," "high-risk customers") and scale each batch. This produces a dataset with natural demographic clusters rather than uniform random variation.

## 10.3 Edge Case Discovery

The most valuable test data is data that triggers bugs. Bugs hide at the edges. LLMs are good at brainstorming edge cases because they have absorbed patterns of what goes wrong in real software. Ask them to think like a destructive tester — generating records that are technically *valid* but represent unusual, extreme, or tricky situations across eight categories: Unicode characters, length extremes, special characters, date boundaries, numeric edges, format variations, cultural variations, and unusual state combinations.
...
Strings break with special characters and encoding. Numbers break at zero, negative values, and extremes. Dates break at month/year boundaries, leap days, and time zones. Enums break with unexpected values. These categories are a reliable guide; the LLM generates the concrete examples.

## 10.4 Persona-Based Test Scenarios

Real users do not interact with software randomly. A 72-year-old retiree navigates a banking app differently than a 25-year-old power user. Persona-based testing creates user profiles with realistic behaviors, goals, and limitations, then generates test scenarios reflecting how each persona would actually use the system.
...
**Never send real production data to an external LLM API.** The safest approach: either use an on-premises LLM that keeps data within your network, use traditional masking tools (Microsoft Presidio, AWS Macie) for the initial pass then use an LLM to enhance realism, or use the LLM to generate a masking strategy that your team applies locally without sending the actual data.

### Privacy-Safe Data Generation

A safer approach: analyze production data *locally* for patterns, then send only the patterns (not the data) to the LLM. Extract field types, min/max/mean values for numbers, average lengths and format patterns for strings, null ratios, and unique value ratios. Send only these statistical patterns to the LLM to generate synthetic records that match the same profile. No PII ever leaves your machine.

## 10.6 Data Privacy and Compliance for Analysts

Generating test data is only half the challenge. The other half is making sure you never inadvertently expose real personal data — to an LLM API, to a test environment, or to a colleague who should not have access. Privacy regulations carry real penalties, and "I was just testing" is not a legal defence.

### GDPR Considerations When Using LLMs

The General Data Protection Regulation applies to any organisation that processes data of EU residents, regardless of where the organisation is based. Three GDPR requirements demand attention.

**Data Processing Agreements (DPAs).** Before sending any data that could contain EU personal data to an LLM provider, your organisation must have a signed DPA with that provider. The DPA specifies what data is processed, how long it is retained, and what happens if there is a breach. Most major providers offer DPAs for enterprise accounts, but the free tier of ChatGPT typically does not qualify. Verify your agreement covers LLM usage specifically.

**Right to erasure (Article 17).** If a data subject requests deletion of their personal data, you must be able to confirm their data was not sent to an LLM, or, if it was, that the provider has deleted it. This is nearly impossible to guarantee with most LLM APIs. The safest approach: never send identifiable personal data to an external LLM.
...
If you work with Protected Health Information (PHI) — patient names, medical record numbers, diagnoses, or treatment dates — HIPAA imposes strict controls.

**Business Associate Agreements (BAAs).** Any LLM provider that processes PHI must sign a BAA with your organisation. As of early 2026, only a handful of LLM platforms offer HIPAA-eligible environments with signed BAAs (notably Microsoft Azure OpenAI Service and AWS Bedrock with specific configurations). Using a consumer LLM product with PHI is a HIPAA violation.

**Minimum necessary standard.** Even with a BAA in place, send only the minimum data necessary for the task. If you need the LLM to generate test cases for a patient scheduling system, send the schema and sample synthetic data, not actual patient records.

**Audit logging.** HIPAA requires logging who accessed PHI, when, and for what purpose. If LLM calls are part of your workflow, those calls must appear in your audit trail.

### CCPA Requirements for California Consumer Data

The California Consumer Privacy Act (and its amendment, CPRA) gives California residents rights over their personal information. Three requirements matter for analysts:

**Disclosure obligations.** If your organisation sends personal information from California residents to an LLM provider for processing, that provider must be disclosed as a "service provider" in your privacy policy.

**Right to delete and right to know.** Consumers can request deletion of their data and ask what data you have collected. You must be able to trace whether their data was included in any LLM processing.

**Do not sell or share.** Sending personal data to an LLM provider whose terms allow them to use your data for model training could be construed as "sharing" under CCPA. Ensure your enterprise agreement explicitly prohibits using your data for training.
...
| 5 | **Confirm data residency.** Where will the data be processed? Does it cross national borders? | Use a regional deployment if data must stay within a jurisdiction |
...
| 10 | **Review retention.** How long will the LLM provider retain your data? | Confirm retention periods align with your data retention policy |

### PII Detection and Masking in Practice

When you need to send data to an LLM but suspect it may contain PII, a simple detection-and-masking step can catch the most common identifiers before they leave your machine:
...
This catches structured identifiers — SSN, email, phone, credit card — but not the name "Jane Doe." Name detection requires a Named Entity Recognition (NER) model. For production use, consider Microsoft Presidio (open source), AWS Comprehend, or Google Cloud DLP. Fast regex pre-screening plus accurate NER post-processing gives you both speed and coverage.
...
The prompt defines the schemas for each system with their foreign key relationships, then describes a specific test scenario — *"A gold-tier customer places two orders: one is delivered successfully, the other is partially shipped with one item backordered."* The LLM generates consistent records across all systems where every foreign key references a valid parent record, timestamps are chronologically ordered, and quantities balance.

Start with the scenario ("customer upgrades their subscription mid-billing-cycle") and generate only the data needed to support it. This produces smaller, more focused datasets that are easier to debug when tests fail.

## 10.8 Data Validation Rules

Beyond generating data, LLMs can help define and enforce the validation rules that your test data and production data must satisfy. This is especially useful when working with legacy systems where validation rules are buried in code and not documented.

The LLM generates validation rules with six attributes: rule\_id, field, rule\_type (format, range, required, dependency, uniqueness, cross-field), description, a validation expression, and severity (error or warning). You apply these rules against your generated records to catch inconsistencies.
...
The validation expressions generated by the LLM can be directly converted into pytest assertions or data quality checks in your ETL pipeline. The same rules that validate your test data can catch data quality issues in production.
...
4. **Privacy-safe pipeline.** Implement the "analyze locally, generate remotely" pattern from Section 10.5. Compare the statistical properties of the synthetic data against the original.
